# h2sense — Passive Exposure Intelligence
...
h2sense strictly separates **Clean Production Mode** from **Deterministic Demo Mode**:
...
- **Safety Disclosures**:
  - h2sense is a **passive cumulative exposure dosimeter**, **NOT** an instantaneous real-time $\text{H}_2\text{S}$ alarm.
...
8. Once provisioned, copy the **Internal Database URL** (or **External Database URL** if hosting backend elsewhere). Example:
   `postgresql://h2sense_user:password@dpg-xxxx.singapore-postgres.render.com/h2sense`

---

### Step 2: Deploy Backend Web Service (Render)
1. In the Render Dashboard, click **New +** $\rightarrow$ **Web Service**.
2. Connect your GitHub repository: `h2sense`.
...
6. Note your deployed Backend URL: `https://h2sense-api.onrender.com`.
7. Verify health: visit `https://h2sense-api.onrender.com/health` $\rightarrow$ should return `{"status": "ok"}`.

---

### Step 3: Deploy Frontend Web Application (Vercel)
1. In the Vercel Dashboard, click **Add New...** $\rightarrow$ **Project**.
2. Import the `h2sense` repository from GitHub.
3. Configure project:
...
4. **Environment Variables**:
   | Variable | Value |
   |---|---|
   | `VITE_API_BASE_URL` | `https://h2sense-api.onrender.com` *(your Render backend URL)* |
5. Click **Deploy**.
6. Once deployed, note your production URL: `https://h2sense.vercel.app`.
7. (Optional): Go back to your Render backend service, and update `ALLOWED_ORIGINS` to `https://h2sense.vercel.app`.

---

## 6. Mobile Phone Testing & Android PWA Installation

1. Open your deployed URL on an Android device: `https://h2sense.vercel.app`.
2. Chrome will display the **"Install h2sense"** banner or option in the menu ($\vdots \rightarrow$ *Add to Home screen*).
...
```bash
# Clone & navigate
git clone https://github.com/your-username/h2sense.git
cd h2sense
